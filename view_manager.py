...
class ViewManager(metaclass=ABCMeta):
    ...
    # 初期化
    def __init__(self):
        self._device = self.get_device(['--config','/home/pi/wisun-gateway/display.conf'])
        ...
        with canvas(self._device) as draw:
            ...
            self._payloadArea = (0, text_h, self._width - 1, self._height - 1 - text_h)
    ...
    def reflesh(self):
        with canvas(self._device) as draw:
            ...
            draw.line((self._payloadArea[0],self._payloadArea[1]-1,self._payloadArea[2],self._payloadArea[1]-1), fill="blue")
            draw.line((self._payloadArea[0],self._payloadArea[3]+1,self._payloadArea[2],self._payloadArea[3]+1), fill="blue")

            # 日付
            now = datetime.datetime.now()
            self._date = now
            dt = u"'{0:02}/{1:02}/{2:02} {3:02}:{4:02}:{5:02}".format(now.year % 100, now.month, now.day, now.hour, now.minute, now.second)
            # date_w, date_h = draw.textsize("'88/88/88 88:88:88", self._font)
            date_w, date_h = draw.textsize(dt, self._font)
            draw.text(((self._width - date_w)/2, self._height - date_h), dt, fill="yellow", font=self._font)
            # IP Address更新
    ...
    def refleshIpAddr(self, draw):
        ...
        if addr is not None:
            ...
            draw.text(((self._width - addr_w)/2, 0), addr, fill="yellow", font=self._font)

    # ペイロード表示のクリア
    def clearPayload(self):
        with canvas(self._device) as draw:
            draw.rectangle((self._payloadArea[0],self._payloadArea[1],self._payloadArea[2],self._payloadArea[3]), fill="black", outline="black")
...
class ViewManagerAnalog(ViewManager):
    ...
    def refleshPayload(self, draw):
        if self._date is not None:
            ...
            # reflet view
            draw.ellipse((cx - radius , cy - radius, cx + radius, cy + radius), outline="white")
