...
class WisunManager(WisunManager):
    ...
    def wisunSendFrame(self, frame: Frame):
        if self._ipv6Addr is not None:
            ...
            cmd = struct.pack('>8sQHHH{0}s'.format(data_len), b'\xfe\x80\x00\x00\x00\x00\x00\x00', self._ipv6Addr ^ 0x0200000000000000, 3610, 3610, data_len, payload)
    ...
    def sendReq(self, cmd, data):
      ...
      hs = sum(buf)
      buf = struct.pack('>{0}sHH{1}s'.format(len(buf), data_len), buf, hs, sum(data), data)
      logger.debug(self._dump(buf))
      c = self._ser.write(buf)
        ...
        while not self._stopReceiveEvent:
            ...
            if len(dt) < state[1]:
                ...
                if state[0] == RCV_IDLE:
                    ...
                    check_data = values[3]
                    logger.debug('check:{0:04x}, sum:{1:04x}'.format(check_header,header_sum))
                    ...
                    data_sum = sum(bar)
                    logger.debug('check:{0:04x}, sum:{1:04x}'.format(check_data,data_sum))
                    if check_data == data_sum:
                        logger.debug('data sum check ok')
                        data = b'' + bar
                        if command == INF_RECV_DATA:
                            head_len = 16 + 2 + 2 + 2 + 1 + 1 + 1 + 2
                            frames = struct.unpack('>16sHHHbbbH', bar[0:head_len])
                            ipAddr = frames[0]
                            srcPort = frames[1]
                            dstPort = frames[2]
                            panId = frames[3]
                            ...
                            rssi = frames[6]
                            data_len = frames[7]
                            logger.info('{0}, srcPort*{1}, dstPort={2}, panId={3}, addrType={4}, encryption={5}, rssi={6}dBm, len={7}byte'.format(ipAddr, srcPort, dstPort, panId, addrType, encryption, rssi, data_len))
    ...
    def _initialize(self, ch):
      # 初期設定
      self.sendReq(CMD_INITIALIZE, struct.pack('BBBB', 0x05, 0x00, ch, 0x00)) # Dual mode, HAN Sleep 無効, チャネル４, 20mW
    ...
    def _activeScan(self):
      ...
      for i in range(retry):
        # アクティブスキャン実行
        self.sendReq(CMD_ACTIVE_SCAN, b'\x09\x00\x03\xff\xf0\x01' + self._bid.encode()[-8:]) # scan 5s, scan ch4-17, PairingIDあり, PairingID(bid last8byte)
          ...
          try:
            ...
            if cmd == RES_ACTIVE_SCAN:
              ...
              if len(res) == 2 and res[0] == 0x01:
                ...
                panId = values[3]
                rssi = values[4]
                logger.info('ch{0} ,mac={1:016x}, panId={2:04x}, rssi={3}'.format(ch, mac, panId, rssi))
              else:
    ...
    def _setBrouteParameter(self):
      # BルートPANA認証情報設定
      self.sendReq(CMD_B_ROUTE_SET_PANA_INFO, struct.pack('{0}s{1}s'.format(len(self._bid), len(self._pwd)), self._bid.encode(), self._pwd.encode()))
    ...
    def _stopBroute(self):
      ...
      for i in range(retry):
        ...
        if self._waitOk('CMD_B_ROUTE_END error', RES_B_ROUTE_END): return True
        # retry
        sleep(2.0)
        return False
    ...
    def _stopBroutePANA(self):
      ...
      for i in range(retry):
        ...
        if self._waitOk('CMD_B_ROUTE_PANA_END error', RES_B_ROUTE_PANA_END): return True
        # retry
        sleep(2.0)
      return False
    ...
    def _startUdp(self, port):
      ...
      for i in range(retry):
        ...
        if self._waitOk('CMD_UDP_OPEN error', RES_UDP_OPEN): return True
        # retry
        sleep(2.0)
      return False
    ...
    def _stopUdp(self, port):
      ...
      for i in range(retry):
        ...
        if self._waitOk('CMD_UDP_CLOSE error', RES_UDP_CLOSE): return True
        # retry
        sleep(2.0)
      return False
    ...
    def _reauthenticationBroutePANA(self):
      ...
      for i in range(retry):
        ...
        if self._waitOk('CMD_B_ROUTE_PANA_RE_AUTH error', RES_B_ROUTE_PANA_RE_AUTH): return True
        # retry
        sleep(2.0)
      return False
    ...
    def connect(self):
        ...
        try:
            if self._smartmeterCh is None:
                # 初期化（任意チャネル）
                if not self._initialize(4): raise ComError('Initialize1')
                sleep(0.2)
                # アクティブスキャン
                values = self._activeScan()
                if values is False: raise ComError('Active Scan')
                self._smartmeterCh = values[0]
            # 初期化（任意チャネル）
            if not self._initialize(self._smartmeterCh): raise ComError('Initialize2')
            sleep(0.2)
            # BルートPANA認証情報設定
            if not self._setBrouteParameter(): raise ComError('B route PANA Setting')
            # Bルート動作開始
            values = self._startBroute()
            if values is False: raise ComError('B route Start')
            # UDPポートOPEN
            port = 3610
            if not self._startUdp(port): raise ComError('UDP({0}) open'.format(port))
            # BルートPANA開始
            if not self._startBroutePANA(): raise ComError('B route PANA Start')
            # 送信タスク開始
            self.startSendTask()
    ...
    def disconnect(self):
        ...
        try:
            ...
            self.stopSendTask()
            # BルートPANA終了
            if not self._stopBroutePANA(): raise ComError('B route PANA Stop')
            # UDPポートCLOSE
            port = 3610
            if not self._stopUdp(port): raise ComError('UDP({0}) close'.format(port))
            # Bルート動作終了
            if not self._stopBroute(): raise ComError('B route Stop')
            # HWリセット
            self.reset()
            sleep(3.0)
        except ComError as err:
