...
class InfluxManager:
    def __init__(self):
        ...
        try:
            ...
            self._client = InfluxDBClient(url, 8086, 'root', 'roor', 'smartmeter')
    ...
    def put(self, properties):
        ...
        for p in properties:
            if p.EPC == 0xe7:
                ...
                text += '-power value={0} {1}\n'.format(values[6] * 0.1, timestamp)
        d = datetime.datetime.today()
        f = open('/home/pi/wisun-gateway/power{0:04d}{1:02d}{2:02d}.txt'.format(d.year,d.month,d.day), 'a')
        f.write(text)
        f.close()
