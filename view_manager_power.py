...
class ViewManagerPower(ViewManager):
    ...
    def __init__(self, iniFile):
        ...
        self._fontTitle = self.make_font(
            self._font_name, int(iniFile.get('view', 'font_power_title')))
    ...
    def refleshPayload(self, draw):
        ...
        font_t = self._fontTitle
        font_p = self._fontPower
        width = self._payloadArea[2] - self._offset_x
        height = self._payloadArea[3] - self._offset_y
        power_h = (height) / 3
        w, h = draw.textsize('瞬', font=font_t)
        unit_w, unit_h = draw.textsize('W', font=font_t)
        x = 2
        # 瞬時電力
        y = height / 2 + power_h * (-1) - h / 2 + self._offset_y
        draw.text((x, y), '瞬時電力:', fill="green", font=font_t)
        draw.text((width - unit_w, y), 'W', fill="green", font=font_t)
        ...
        draw.text((x, y), '電流Ｒ相:', fill="green", font=font_t)
        draw.text((width - unit_w, y), 'A', fill="green", font=font_t)
        ...
        # 電流T相
        y = height / 2 + power_h * (1) - h / 2 + self._offset_y
        draw.text((x, y), '電流Ｔ相:', fill="green", font=font_t)
        draw.text((width - unit_w, y), 'A', fill="green", font=font_t)
